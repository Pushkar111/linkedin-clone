...
export const getAllPosts = asyncHandler(async (req, res) => {
  ...
  // Add currentUserReaction field for each post
  posts = posts.map(post => {
    if (userId && post.reactions) {
      const userReaction = post.reactions.find(r => r.user.toString() === userId.toString());
      post.currentUserReaction = userReaction ? userReaction.type : null;
    } else {
...
export const toggleReaction = asyncHandler(async (req, res) => {
  ...
  let message = '';
  if (result.reacted && currentReaction && currentReaction.type !== reactionType) {
  ...
  // Return consistent response format (matching comment reactions)
  res.status(200).json({
    success: true,
    message,
    data: {
      ...
      reactionCounts: result.reactionCounts,
      totalReactions: result.reactionCount,
    },
  });
});
...
export const getUserPosts = asyncHandler(async (req, res) => {
  ...
  posts = posts.map(post => {
    if (currentUserId && post.reactions) {
      const userReaction = post.reactions.find(r => r.user.toString() === currentUserId.toString());
      post.currentUserReaction = userReaction ? userReaction.type : null;
    } else {
      post.currentUserReaction = null;
    }
...
/**
 * @route   GET /api/posts/reactions/me
 * @desc    Get all reactions for current user (for persistence across refresh)
 * @access  Private
 */
export const getUserReactions = asyncHandler(async (req, res) => {
  const userId = req.user._id;
  ...
  }).filter(r => r.reactionType); // Filter out any nulls
