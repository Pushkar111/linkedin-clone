  ...
  try {
    ...
    return {
      posts: posts.map(post => ({
        ...
        // Author info
        author: {
          _id: post.user?._id || post.author?._id || post.user || post.author,
          name: post.user?.fullName || post.author?.fullName || post.author?.name,
          fullName: post.user?.fullName || post.author?.fullName || post.author?.name,
          profilePicURL: post.user?.profilePicURL || post.author?.profilePicURL || post.author?.avatarUrl,
          headline: post.user?.profile?.headline || post.author?.profile?.headline || post.author?.headline
        },
